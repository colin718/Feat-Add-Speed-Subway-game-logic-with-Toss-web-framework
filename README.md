# Feat-Add-Speed-Subway-game-logic-with-Toss-web-framework
HTML5, CSS3, Vanilla JS를 활용한 지하철 역 맞추기 게임. 토스 챌린지 참여를 위해 Apps-in-toss SDK를 연동하고 리더보드 기능을 구현함.
[SpeedSubway.html](https://github.com/user-attachments/files/24204899/SpeedSubway.html)
<!doctype html>
<html lang="ko">
<head>
<meta charset="utf-8"/>
...
<title>Speed Subway — 다음 역 맞추기</title>
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  body {
    ...
    background-image: url('background.jpg'); /* 배경 이미지 경로 지정 */
    background-size: cover;                  /* 화면 비율에 맞게 꽉 채우기 */
    background-position: center;             /* 이미지를 중앙에 위치 */
    background-repeat: no-repeat;            /* 이미지가 반복되지 않도록 설정 */
    background-color: #2e353b;               /* 이미지가 로딩되지 않을 때 보일 배경색 */
}
  ...
  .subway-wall {
    ...
    width: 100%;
    height: 100%;
    /* 배경 이미지 관련 코드를 다시 제거하여 투명하게 만듭니다. */
    background: transparent; /* 이 부분 추가 */
    display: flex;
  ...
  #intro {
    text-align: center;
    margin: 0 auto;
    background: transparent; /* 배경을 완전히 투명하게 변경 */
    padding: 40px;
    border-radius: 15px;
  ...
  .station-card.next-card {
    width: 220px; /* ✨ 양쪽 카드 너비 (원하는 크기로 조절하세요) */
  }

  .station-card.current {
    width: 400px; /* ✨ 가운데 카드 너비 (원하는 크기로 조절하세요) */
  }
  ...
  .station-row {
    display: flex;
    justify-content: center; /* 좌-중-우 배치 */
    align-items: center;
    width: 100%;
    max-width: var(--max-width);
    gap: 24px; /* 카드 사이 간격 */
  ...
  /* 움직이는 지하철 스타일 및 애니메이션 */
  #animatedTrain {
    position: absolute; /* 부모 요소(.subway-wall) 기준으로 위치 고정 */
    bottom: -880px;
    left: 200%;
    width: 2200px;
    z-index: 15; /* billboard(z-index:2)보다 뒤에 위치 */
    
    animation: slideInAndStop 22s ease-in-out infinite;
  }
    /* 애니메이션 레이어 스타일 (없다면 추가) */
  .animation-layer {
    position: absolute;
    top: 0;
    left: 0;
    ...
    z-index: 5; /* 전광판(billboard)과 배경 사이 적절한 위치 */
  }
  ...
  #waitingPerson1, #waitingPerson2 {
    ...
    z-index: 3;
    top: 425px;
    /* left: 50%를 공통 스타일에서 제거합니다. */
  }
  
  #waitingPerson1 {
    /* 이제 고정된 너비(800px)를 기준으로 위치가 계산됩니다. */
    left: 200px; 
  }
  ...
  /* 지하철 코드도 position: absolute; 인지 다시 한번 확인합니다. (현재 코드는 올바르게 되어 있습니다.) */
  #animatedTrain {
    position: absolute; /* ✅ 부모(.animation-layer) 기준 */
    bottom: -880px;
    left: 210%;
    width: 2200px;
    z-index: 15;
  ...
  /* 지하철이 들어와서 멈추는 움직임 정의 */
  @keyframes slideInAndStop {
    /* --- 0% ~ 55% (약 12초): 기존처럼 열차가 보이고 움직이는 구간 --- */
    0% {
      transform: translateX(0%); /* 화면 오른쪽 밖에서 출발 */
    }
    14% { /* (기존 25% 지점) */
      transform: translateX(-110%); /* 화면 안으로 들어와 정차 */
    }
    45% { /* (기존 80% 지점) */
      transform: translateX(-110%); /* 정차 상태 유지 */
    }
    55% { /* (기존 100% 지점) */
      transform: translateX(-220%); /* 화면 왼쪽 밖으로 사라짐 */
    }
  
    /* --- 55% ~ 100% (약 10초): 열차가 화면 밖에 머무르는 지연 구간 --- */
    100% {
      transform: translateX(-220%); /* 사라진 위치에서 계속 대기 */
    }
  }
  ...
  .interactive-area {
    /* 그룹 내부 요소(역명판, 메시지, 선택지)를 정렬하기 위한 코드 */
    width: 100%;
    display: flex;
    flex-direction: column;
    align-items: center;
    ...
    /* 상단 바(timer)와의 간격을 주어 그룹 전체를 아래로 이동시키는 코드 */
    margin-top: 60px; /* ✨ 이 값을 조절해서 원하는 높이로 맞추세요 */
  }

  @media (max-width: 640px) {
    /* 모바일에서는 전광판 높이를 조금 줄입니다. */
    .billboard {
      height: 380px; 
    }
  
    /* 모바일에서는 사람과 지하철 크기를 약간 줄여 비율을 맞춥니다. */
    #animatedTrain {
      transform: scale(0.8);
      /* 크기가 작아졌으므로 위치도 미세 조정이 필요할 수 있습니다. */
      bottom: -750px; 
    }
    #waitingPerson1, #waitingPerson2 {
      transform: scale(0.85); /* 크기만 줄이고 */
      bottom: -45px;
      top: auto; /* 세로 위치 충돌 방지 */
    }

    #waitingPerson1 {
        left: 20%; /* 좌우 위치는 여기서 다시 지정합니다. */
    }

    #waitingPerson2 {
        right: 20%; /* 좌우 위치는 여기서 다시 지정합니다. */
    }
  }
...
<body>
  <select id="line-select" aria-label="노선 선택" style="display:none;"></select>

  <div id="loadingOverlay" role="status" aria-live="polite" aria-busy="true">
    <div id="loadingCard">
      <div class="spinner" aria-hidden="true"></div>
      <div id="loadingText" style="font-weight:800;">역 데이터 로드 중...</div>
    </div>
  </div>
  ...
  <div class="wrap">
    ...
    <div id="intro" role="dialog" aria-modal="true">
      <div class="animation-layer">
        ...
        <img id="animatedTrain" src="train.png" alt="움직이는 지하철">
        <img id="waitingPerson1" src="human1.png" alt="기다리는 사람">
        <img id="waitingPerson2" src="human2.png" alt="기다리는 사람">
        </div>
      </div>
      <div class="subway-wall">
        <div class="billboard-content">
          <div id="mode-selection" style="margin-bottom:32px;">
            <div style="color:#fff; font-weight:800; font-size:24px; margin-bottom:20px; text-align:center; text-shadow:0 2px 4px rgba(0,0,0,0.5);">게임 모드 선택</div>
            <div style="display:flex; gap:20px; justify-content:center; margin-bottom:20px;">
              <button id="mode1-btn" class="mode-select-btn selected" aria-pressed="true">
                <div style="font-size:18px; font-weight:700;">타임어택</div>
                <div style="font-size:14px; color:#00ffcc; margin-top:6px;">1분 제한</div>
              </button>
              <button id="mode2-btn" class="mode-select-btn" aria-pressed="false">
                <div style="font-size:18px; font-weight:700;">목적지 도달</div>
                <div style="font-size:14px; color:#00ffcc; margin-top:6px;">여러 노선</div>
              </button>
            </div>
          </div>
          <div id="line-selection-container" style="display:none; margin-bottom:24px;">
            <div style="color:#fff; font-weight:800; font-size:20px; margin-bottom:12px; text-align:center; text-shadow:0 2px 4px rgba(0,0,0,0.5);">노선 선택</div>
            <div id="line-selection-info" style="color:#00ffcc; font-size:16px; margin-bottom:16px; text-align:center;">플레이할 노선을 선택하세요</div>
            ...
            <div style="margin-top:20px; text-align:center;">
              <div id="button-status" style="color:#00ffcc; font-size:14px; margin-top:10px; font-weight:600;">노선을 선택해주세요</div>
            </div>
          </div>
          <button id="start-btn" disabled>게임 시작</button>
          <div id="start-info" style="color:#00ffcc; font-weight:700; margin-top:8px; text-align:center;">노선 로드가 끝나면 시작 버튼이 활성화됩니다.</div>
        </div>
      </div>
      ...
      <div class="topbar" aria-hidden="false">
        ...
        <div class="timer-box">
          ...
          <div class="best" id="bestTime">최고기록: -</div>
        </div>
      </div>
      <div class="interactive-area">
        <div class="station-row" role="region" aria-label="현재역 표시">
        ...
        <div id="msg" aria-live="assertive">노선을 선택하고 시작 버튼을 눌러주세요.</div>
        <div id="options" role="list"></div>
      </div>
      <div class="spacer"></div>
    </div> </div> <script>
    /* ... (기존 script 내용은 여기에 그대로 들어갑니다) ... */
  </script>
  <script type="module">
    /* ... (기존 script type="module" 내용은 여기에 그대로 들어갑니다) ... */
  </script>
...
<script>
/* ===============================
  전체 기능: (기존 코드 기반, 아래 항목 변경)
  1) 로딩 상태 표시 (loadingOverlay)
  2) 리더보드 제출: 지나간 역 수 (movedStations)
  3) 타임어택 종료 조건 구분(타임아웃/순환선 완주/종점)
  4) 접근성 메시지 다양화 (정답/오답/종료 메시지 랜덤)
  5) 브랜드 JH 상단에 추가 (HTML)
  =============================== */
...
let timeLimit = 60000; // 1분 for mode 1

// 순환선/일반선 구분, 시작 인덱스
let startStationIndex = 0;
let isLoopLine = false;

// 트래킹: 지나간 역 수 (leaderboard 제출값)
window.movedStations = 0; // 전역으로 노출 (module 스크립트에서 사용)
let movedStations = 0;
...
const LINE_COLORS = {
  ...
  '우이신설선':'#B7C452',
  '신림선':'#6789CA',
  '서해선': '#81A64B', // 서해선 색상 추가
  '김포골드라인': '#A17800', // 김포골드라인 색상 추가
  '인천1호선': '#7CA8D5', // 인천1호선 색상 추가
  '인천2호선': '#ED8B00', // 인천2호선 색상 추가
  '의정부': '#FFB200' // 의정부 경전철 색상 추가
};
...
const FINISH_MSGS = {
  ...
  loop: [
    '⭕ 축하합니다 — 순환선을 한 바퀴 완주했습니다!',
    '순환선 완주! 게임을 성공적으로 마쳤습니다.',
    '한 바퀴 돌았어요! 정말 빠릅니다.'
  ],
  terminal: [
    '🏁 종점 도달! 게임 종료.',
...
fetch(JSON_PATH)
  ...
  .then(data => {
    ...
    isDataLoaded = true;
    msgEl.textContent = '노선을 선택하세요.';
    // enable start after small delay so intro animation finishes nicely
    setTimeout(()=> {
      startBtn.disabled = true; // 기본적으로 비활성화 (선택 필요)
  ...
  .catch(err => {
    ...
    loadingText.textContent = '데이터를 불러오지 못했습니다. (json 파일 경로 확인)';
    loadingOverlay.style.background = 'rgba(0,0,0,0.6)';
    msgEl.textContent = '데이터를 불러오지 못했습니다.';
    // keep overlay visible so user notices error
  });

function pickStationNames(row){
  const v = row.역명 ?? row.station ?? row.name;
  if(!v) return { ko: '', en: '' };
...
function buildStructureFromArray(arr){
  ...
  for(const it of arr){
    const region = it.권역명 ?? it.권역 ?? '전체';
    const line   = it.노선명 ?? it.노선 ?? '기타';
  ...
  for(const [key, items] of map.entries()){
    ...
    const sorted = items.slice().sort((a,b)=> Number(a.순번)-Number(b.순번));
    const names  = [];
...
function renderStation(){
  ...
  // 게임 종료 시점 (종점)에서는 다음 역을 '종점'으로 표시
...
function renderOptionsPlaceholder(){ optionsEl.innerHTML = ''; if(!currentLine.length) return; const note = document.createElement('div'); note.style.gridColumn = '1 / -1'; note.style.textAlign='center'; note.style.color='var(--muted)'; note.style.padding='12px'; note.style.fontWeight='700'; note.textContent = '게임 시작 후 선택지가 표시됩니다.'; optionsEl.appendChild(note); }
...
mode1Btn.addEventListener('click', ()=>{
  ...
  lineSelectionInfo.textContent = '플레이할 노선을 선택하세요';
...
mode2Btn.addEventListener('click', ()=>{
  ...
  lineSelectionContainer.style.display = 'block';
  lineSelectionInfo.textContent = '연결된 노선들을 선택하세요 (2개 이상)';
  populateLineSelectionGrid();
  updateStartButtonState();
});
...
function startGame() {
  if(!isDataLoaded){
    alert('데이터 로드가 아직 끝나지 않았습니다.');
    return;
  }

  if(gameMode === 'timeattack') {
    if(selectedLines.length === 0){
      alert('플레이할 노선을 선택하세요.');
      return;
    }
    // 선택된 노선으로 현재 노선 설정
    ...
    if(selectedLines.length < 2) {
      alert('최소 2개 이상의 노선을 선택해주세요.');
      return;
    }
    if(!validateConnectedLines()) {
      alert('선택한 노선들이 연결되어 있지 않습니다. 환승이 가능한 노선들을 선택해주세요.');
      return;
    }
...
function updateStartButtonState() {
  ...
  if(!isDataLoaded){
    ...
    if(buttonStatus) buttonStatus.textContent = '데이터 로드 중...';
    return;
  }
  ...
  if(gameMode === 'timeattack') {
    ...
    if(buttonStatus) {
      buttonStatus.textContent = shouldEnable ? '게임을 시작할 수 있습니다!' : '노선을 선택해주세요';
    }
  } else if(gameMode === 'destination') {
    ...
    if(buttonStatus) {
      if(selectedLines.length === 0) {
        buttonStatus.textContent = '노선을 선택해주세요';
      } else if(selectedLines.length === 1) {
        buttonStatus.textContent = '2개 이상의 노선을 선택해주세요';
      } else {
        buttonStatus.textContent = '게임을 시작할 수 있습니다!';
      }
    }
  }
}
...
function startTimeAttackMode() {
  ...
  // 랜덤 시작 (마지막 역은 제외해야 다음 역이 존재)
  currentIndex = Math.floor(Math.random() * currentLine.length);

  // 게임 시작 시 순환선 여부와 시작 인덱스 설정
  startStationIndex = currentIndex;
  isLoopLine = (lineName === "2호선(순환선)"); // 필요하면 더 많은 순환선 이름을 추가
  ...
  finishReason = null;
  started = true;
  msgEl.textContent = `${lineName} 타임어택 모드 시작! 1분 안에 최대한 멀리 가세요!`;
  renderStation();
  fillOptions();
  startTimer();
  initAudio();
...
function updateSelectedLinesDisplay() {
  ...
  displayEl.innerHTML = ''; // 내용을 비웁니다.

  // '목적지 도달' 모드이고, 선택된 노선이 있을 때만 표시합니다.
...
function startDestinationMode() {
  ...
  started = true;
  msgEl.textContent = `${startStation.ko}에서 ${endStation.ko}까지 가세요!`;
  renderStation();
  fillOptions();
  startTimer();
  initAudio();
...
for (const lineKey of selectedLines) {
  ...
  if (Math.abs(currentInThisLineIdx - stationIndex) === 1) {
    ...
    if(found) {
      ...
      setTimeout(()=>{
        fillOptions();
        msgEl.textContent = `${destinationStation.ko}까지 계속 가세요!`;
      }, 240);
    } else {
      playFail();
      domEl.classList.add('wrong');
...
function finishTimeAttackMode(){
  ...
  // 상세 메시지 표시
  msgEl.textContent = `${text} ${distance}개 역을 지나갔습니다. 기록: ${formatTime(elapsed)}`;

  // 로컬 최고거리 저장 (TimeAttack은 '많을수록 좋음')
  ...
  n.innerHTML = `${text}<br>${distance}개 역을 지나갔습니다.<br>다른 노선에 도전하세요!`;
  ...
  retryBtn.textContent = '같은 조건으로 재시도';
...
function finishDestinationMode(){
  ...
  // 메시지
  msgEl.textContent = `🎯 목적지 도달! ${distance}개 이동. 기록: ${formatTime(elapsed)}`;

  // 목적지 모드는 시간 기반 최고기록 저장(짧을수록 좋음)
  ...
  n.style.fontWeight=800;
  n.style.padding='18px';
  n.innerHTML = `목적지에 도달했습니다!<br>${destinationStation.ko}까지 ${formatTime(elapsed)}만에 도착! (이동: ${distance}회)<br>다른 경로에 도전하세요!`;
  optionsEl.appendChild(n);
...
function updateBest(){
  if(gameMode === 'timeattack'){
    ...
    bestEl.textContent = v ? `타임어택 최고거리: ${v} 역` : '타임어택 최고거리: -';
  } else if(gameMode === 'destination'){
    const k = `speed_subway_destination__${selectedLines.join('_')}`;
    const v = localStorage.getItem(k);
    bestEl.textContent = v ? `목적지 도달 최고기록: ${formatTime(Number(v))}` : '목적지 도달 최고기록: -';
  } else {
    bestEl.textContent = '-';
  }
}
...
<script type="module">
  ...
  try {
    bedrockEvent.addEventListener('backEvent', {
      onEvent: () => {
        const ok = confirm('게임을 종료하시겠습니까? 진행 중인 기록은 저장되지 않을 수 있습니다.');
        if (ok) {
          window.close();
        }
      },
  ...
  // finishTimeAttackMode 패치: elapsed 대신 movedStations 제출
  ...
  // finishDestinationMode 패치: 목적지 모드에서도 이동 횟수로 제출 (요청대로)
